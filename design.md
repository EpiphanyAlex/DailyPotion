...
设计稿源文件：Pencil（`design/dailypotion.pen`）。本文件是 DailyPotion 设计 token 的唯一权威定义（single source of truth）。前端实现、设计稿和导出参考图必须引用这里的 token，禁止硬编码色值、字号、间距、圆角或阴影。
...
| `--color-ink-faint` | `#8A7B66` | 元信息、占位符、未激活图标 |
| `--color-accent` | `#7E2D26` | 主操作、active nav、selected filter |
| `--color-on-accent` | `#F9F3E4` | accent/success 深色背景上的文字 |
| `--color-gold` | `#A97E2F` | kicker、细线、少量图标点缀 |
| `--color-border` | `#D8CBAE` | 1px 描边、分隔线 |
...
| `--color-focus-ring` | `rgba(126, 45, 38, 0.35)` | 键盘焦点环（2px outline + 2px offset） |
...
| `--color-info` | `#4E6E7E` | 信息提示图标 / 文字、onboarding 引导 |
...
- 金色只用于 kicker、细线和少量 icon；不要大面积铺色。
...
| Token | Size | Usage |
|---|---:|---|
| `--text-recipe-title` | `28-36px` | Recipe title / 今日推荐主配方 |
| `--text-page-title` | `24-32px` | 页面标题 |
| `--text-card-title` | `16-18px` | 卡片标题、列表实体名 |
| `--text-body` | `14-16px` | 正文、描述、配料、步骤 |
| `--text-caption` | `12px` | meta、说明、secondary label |
| `--text-micro` | `10-12px` | kicker、badge、tab label，uppercase + letter spacing |
...
- Micro/kicker：10-12，uppercase，letter-spacing 1.5-2px，`color-gold`。
...
- Desktop option A：top navigation with logo left；nav links Today / Recipes / Cabinet / History / Favorites；search input；language switch；profile/avatar。
- Desktop option B：premium sidebar shell with logo, Home / Daily Pour / Recipes / My Cabinet / Favorites / History / Settings, plus a compact cabinet summary. Use this when the app needs more dashboard density, as in the updated reference art.
- Active nav：`color-accent` 文字或浅 `paper-selected` 背景，必要时加 1px `border`。
- Search：`paper-raised` 背景、`border`、`radius-pill`、search icon、placeholder 使用 `ink-faint`。
...
- Recipe Card：图片、名称、中文名、基酒/风味标签、rating、Can Make/Missing status、favorite icon。
...
1. Top Navigation：logo、主导航、search、language、profile。
   Desktop may use the sidebar shell variant when the dashboard needs persistent product navigation and cabinet context.
2. Hero: Today’s Daily Pour：约占首屏 40-45% 高度；左侧真实鸡尾酒图片，右侧配方名/中文名/描述/tags/时间/难度/ABV；操作包括 Can Make、View Recipe、Save。
3. Cabinet Snapshot：Bottles Owned 18、Cocktails You Can Make 126、Missing Only 2、Cabinet Completion 72%、CTA Go to Cabinet。
4. Because You Have Roku Gin：横向推荐卡片 Martini、Tom Collins、Bee’s Knees、Gimlet、White Lady；每张显示图片、名称、中文名、rating、Can Make、favorite。
...
2. Hero Area：真实鸡尾酒图；Recipe name / 中文名；description；tags Classic / Medium / Strong；meta 5 min / 24% ABV / Easy。
3. Ingredients + Instructions：双栏卡片（桌面）；移动端上下堆叠。行高清晰，配料和步骤使用分隔线。
4. Availability Panel：明确显示 You have all ingredients，或 Missing: Campari, Sweet Vermouth。
5. Actions：Log Your Pour、Add to Favorites、Rate this recipe。
6. Bartender Tip：小型提示卡，不抢主流程。
...
- Page title + Search。
- Filter chips：All Spirits / Gin / Whisky / Rum / Vodka / Tequila / Can Make / Favorites。
...
- 表单规格：label 用 `font-ui` caption 600 `ink-soft`；输入框高 44、`paper-raised` + `border` + `radius-sm`；焦点态 3px `focus-ring`；错误态 `danger-soft` 底 + 1.5px `danger` 边 + 下方 icon+`danger` 文案；未通过校验时主按钮为禁用态（`surface-disabled` + `ink-disabled`）。
- 第三方登录（Google）用 Secondary 按钮样式；分隔线"或"用 `border` 细线。
...
- **Modal**：`paper-raised` + `border` + `radius-md` + `shadow-floating`，padding 20；标题 Playfair title + 右上 X；页面背景加 `image-overlay` 遮罩。
  ...
  - Log Your Pour Modal：配方名 + 日期、金色星级（未选星用 `ink-disabled`）、口味 tag chips（选中态 `paper-selected` + `accent` 边）、笔记 textarea、取消/保存按钮组右对齐。
...
- Bottom Tab：Home / Recipes / Cabinet / Favorites / Profile。
- Home mobile：compact Daily Pour card、Cabinet snapshot cards、horizontal recipe carousel。
