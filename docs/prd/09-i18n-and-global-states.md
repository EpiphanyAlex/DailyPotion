...
### 2.4 运行时显示策略（哪些内容双语并排）

设计稿中大面积中英并排是画稿示意，**不是运行时规则**（design.md 已注明）。运行时按以下策略显示：

- **导航、按钮、说明文、配方步骤、表单、toast 等一切 UI 文案与长文本**：只显示当前语言（fallback 规则见 §2.3）。
- **实体名（配方名、酒瓶名）**：主名 = 当前 locale 列；可**选配**展示另一语言列作为次要别名（小号、弱化样式，规格见 design.md）。别名是增强项，不是必须项；当前 locale 列为空时按 §2.3 fallback，另一语言列直接升为主名、不再重复展示别名。
- **搜索**：始终同时匹配 `*_zh` 与 `*_en`（见 [配方库](05-recipes-library.md) 搜索规则）。
...
- UI 文案（导航/按钮/说明/步骤）不出现双语并排；实体名主名为当前语言，次要别名（如展示）为弱化样式（§2.4）。
- 中文站搜索英文名（或反向）能命中对应配方/酒瓶。
